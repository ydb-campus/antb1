...
// AST of the supported SQL subset (docs/sql-subset.md). Every node carries the span it was parsed
// from; EqualIgnoringSpans() compares structure only (used by the parse(unparse(x)) == x fuzz
// property).
...
struct Literal {
  ...
  std::string text;       // digits / unescaped string / date text as written
  SourceSpan span;
};
