...
# drift/policy checks (.pre-commit-config.yaml), then the tooling tests (tools/lint/tests). `pixi run fmt` fixes
...
python -m pytest -q -p no:cacheprovider tools/lint/tests || rc=1
