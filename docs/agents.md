...
| `Claude review` ([claude-review.yml](../.github/workflows/claude-review.yml)) | PR opened, ready for review or reopened (same repository, not a draft, no bots except Copilot) | Anthropic's code-review plugin posts inline comments for high-confidence problems, or one "No issues found" comment | `CLAUDE_CODE_OAUTH_TOKEN` |
